    ...
    login(userid, password, response){
        UserModel.findOne({'userid':userid,'password':password},(err,doc)=>{
            if(err){
                ...
                if(doc){
                    let tokenNumber = token.generateToken(userid);
                   response.status(200).json({"token":tokenNumber,"message":"Welcome","userid":userid,"role":doc.role,"status":config.SUCCESS}) ;
